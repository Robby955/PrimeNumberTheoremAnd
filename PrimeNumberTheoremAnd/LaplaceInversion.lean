...
This file proves a Fourier-inversion form of bilateral Laplace inversion.
The statement is independent of the Kadiri application and is shaped for later
Mathlib upstreaming.
-/
...
/-- The inverse bilateral Laplace line integral on `re s = sigma`. -/
def laplaceInvLine (sigma : ℝ) (F : ℂ → E) (x : ℝ) : E :=
  (1 / (2 * π) : ℝ) •
    ∫ t : ℝ, Complex.exp (((sigma : ℂ) + (t : ℂ) * I) * (x : ℂ)) •
      F ((sigma : ℂ) + (t : ℂ) * I)

/-- The truncated inverse Fourier integral in the normalization used by
`laplaceInvLine`. This is the finite-height object whose limit is needed for
principal-value Laplace inversion. -/
def fourierInvTrunc (F : ℝ → E) (x T : ℝ) : E :=
  (1 / (2 * π) : ℝ) •
    ∫ t in (-T)..T, Complex.exp (((t * x : ℝ) : ℂ) * I) • F (t / (2 * π))

/-- The truncated inverse bilateral Laplace line integral on `re s = sigma`. -/
...
/-- The inverse Fourier transform after the `t = 2πy` change of variables. -/
theorem fourierInv_eq_integral_comp_div (F : ℝ → E) (x : ℝ) :
    𝓕⁻ F x =
      (1 / (2 * π) : ℝ) •
        ∫ t : ℝ, Complex.exp (((t * x : ℝ) : ℂ) * I) • F (t / (2 * π)) := by
  rw [Real.fourierInv_eq']
  have h := MeasureTheory.Measure.integral_comp_mul_left
    (fun t : ℝ => Complex.exp (((t * x : ℝ) : ℂ) * I) • F (t / (2 * π))) (2 * π)
  have habs : |(2 * π)⁻¹| = (1 / (2 * π) : ℝ) := by
    rw [abs_of_pos]
    · ring
    · positivity
  rw [← habs, ← h]
...
/-- Expanding the Fourier transform inside the finite-height inverse Fourier
integral gives the standard oscillatory kernel before the Fubini step. -/
...
/-- Finite-height Fourier inversion rewritten with the explicit Dirichlet
kernel. This is the kernel form needed for principal-value cancellation. -/
theorem fourierInvTrunc_fourier_eq_dirichlet_kernel
    [CompleteSpace E] {f : ℝ → E} (hf : Integrable f) (x T : ℝ) (hT : 0 ≤ T) :
    fourierInvTrunc (𝓕 f) x T =
      (1 / (2 * π) : ℝ) •
        ∫ y : ℝ,
          (if x - y = 0 then (2 * T : ℂ)
            else (2 * Real.sin (T * (x - y)) : ℂ) / ((x - y : ℝ) : ℂ)) • f y := by
...
/-- Finite-height Fourier inversion rewritten with the scaled `sinc` kernel. -/
...
/-- Principal-value Fourier inversion from convergence of the explicit
Dirichlet-kernel integrals. This isolates the remaining analytic input. -/
theorem fourierInvTrunc_tendsto_of_dirichlet_kernel
    ...
    (hdir : Filter.Tendsto
      (fun T : ℝ =>
        (1 / (2 * π) : ℝ) •
          ∫ y : ℝ,
            (if x - y = 0 then (2 * T : ℂ)
              else (2 * Real.sin (T * (x - y)) : ℂ) / ((x - y : ℝ) : ℂ)) • f y)
      Filter.atTop (nhds (f x))) :
    Filter.Tendsto (fun T : ℝ => fourierInvTrunc (𝓕 f) x T)
      Filter.atTop (nhds (f x)) := by
  refine hdir.congr' ?_
  filter_upwards [Filter.eventually_ge_atTop (0 : ℝ)] with T hT
  exact (fourierInvTrunc_fourier_eq_dirichlet_kernel hf x T hT).symm

/-- Principal-value Fourier inversion from convergence of the scaled `sinc`
kernel integrals. This is equivalent to the Dirichlet-kernel carry but avoids a
removable-singularity case split. -/
...
/-- A sinc-kernel limit follows once the kernel has been rewritten as its value at
the target point plus an integrable sine-error term. This packages the remaining
principal-value algebra separately from the Riemann-Lebesgue cancellation. -/
theorem sinc_kernel_tendsto_of_integrable_sine_error
    {f : ℝ → E} {x : ℝ} {q : ℝ → E} (hq : Integrable q)
    (hrep : (fun T : ℝ =>
        (1 / (2 * π) : ℝ) •
          ∫ y : ℝ, (2 * T * Real.sinc (T * (x - y)) : ℂ) • f y) =ᶠ[Filter.atTop]
        (fun T : ℝ => f x + ∫ u : ℝ, (Real.sin (T * u) : ℂ) • q u)) :
    Filter.Tendsto
      (fun T : ℝ =>
        (1 / (2 * π) : ℝ) •
          ∫ y : ℝ, (2 * T * Real.sinc (T * (x - y)) : ℂ) • f y)
      Filter.atTop (nhds (f x)) := by
  have hsin := tendsto_integral_sin_mul_smul_atTop (E := E) hq
  have hmain : Filter.Tendsto
      (fun T : ℝ => f x + ∫ u : ℝ, (Real.sin (T * u) : ℂ) • q u)
      Filter.atTop (nhds (f x + 0)) := by
    exact tendsto_const_nhds.add hsin
  simpa using hmain.congr' hrep.symm

/-- Principal-value Fourier inversion from a sinc-kernel representation whose
only nonconstant term is an integrable sine-error integral. -/
theorem fourierInvTrunc_tendsto_of_sinc_error
    [CompleteSpace E] {f : ℝ → E} (hf : Integrable f) {x : ℝ} {q : ℝ → E}
    (hq : Integrable q)
    (hrep : (fun T : ℝ =>
        (1 / (2 * π) : ℝ) •
          ∫ y : ℝ, (2 * T * Real.sinc (T * (x - y)) : ℂ) • f y) =ᶠ[Filter.atTop]
    ...
    Filter.Tendsto (fun T : ℝ => fourierInvTrunc (𝓕 f) x T)
      Filter.atTop (nhds (f x)) := by
  exact fourierInvTrunc_tendsto_of_sinc_kernel (E := E) hf
    (sinc_kernel_tendsto_of_integrable_sine_error (E := E) hq hrep)

/-- A sinc-kernel limit follows from an explicit `sin (T * u) / (π * u)`
representation, after removing the singleton value at `u = 0`. -/
theorem sinc_kernel_tendsto_of_sin_div_error
    {f : ℝ → E} {x : ℝ} {q : ℝ → E} (hq : Integrable q)
    (hrep : (fun T : ℝ =>
        ∫ u : ℝ,
          (if u = 0 then (0 : ℂ) else (Real.sin (T * u) / (π * u) : ℂ)) •
    ...
    Filter.Tendsto
      (fun T : ℝ =>
        (1 / (2 * π) : ℝ) •
          ∫ y : ℝ, (2 * T * Real.sinc (T * (x - y)) : ℂ) • f y)
      Filter.atTop (nhds (f x)) := by
  refine sinc_kernel_tendsto_of_integrable_sine_error (E := E) hq ?_
  filter_upwards [hrep] with T hT
  rw [normalized_sinc_kernel_integral_comp_sub_left_sin_div_ae]
  exact hT

/-- Principal-value Fourier inversion from an explicit `sin (T * u) / (π * u)`
representation whose only nonconstant term is an integrable sine-error
integral. -/
theorem fourierInvTrunc_tendsto_of_sin_div_error
    [CompleteSpace E] {f : ℝ → E} (hf : Integrable f) {x : ℝ} {q : ℝ → E}
    (hq : Integrable q)
    (hrep : (fun T : ℝ =>
        ∫ u : ℝ,
    ...
    Filter.Tendsto (fun T : ℝ => fourierInvTrunc (𝓕 f) x T)
      Filter.atTop (nhds (f x)) := by
  exact fourierInvTrunc_tendsto_of_sinc_kernel (E := E) hf
    (sinc_kernel_tendsto_of_sin_div_error (E := E) hq hrep)

/-- Pointwise algebra for the principal-value error term after subtracting the
target value. -/
...
/-- Integral form of the principal-value quotient algebra. -/
theorem integral_sin_div_kernel_sub_eq_sin_smul_quotient
    (f : ℝ → E) (x T : ℝ) :
    (∫ u : ℝ,
        (if u = 0 then (0 : ℂ) else (Real.sin (T * u) / (π * u) : ℂ)) •
          (f (x - u) - f x)) =
...
/-- The local quotient in the sine-kernel error agrees off the origin with a
translated `dslope`. -/
...
/-- The finite one-sided sinc windows are Cauchy along natural heights. -/
theorem cauchySeq_intervalIntegral_real_sinc_nat :
    CauchySeq (fun n : ℕ => ∫ x in (0 : ℝ)..(n : ℝ), Real.sinc x) := by
  refine cauchySeq_of_dist_le_of_one_le
      (s := fun n : ℕ => ∫ x in (0 : ℝ)..(n : ℝ), Real.sinc x)
      (b := fun n : ℕ => 3 * ((n : ℝ)⁻¹)) ?hb ?hb0 ?h
  · intro n
    positivity
  · have h_inv : Filter.Tendsto (fun n : ℕ => ((n : ℝ)⁻¹)) Filter.atTop (nhds 0) :=
      tendsto_inv_atTop_nhds_zero_nat
    simpa using h_inv.const_mul (3 : ℝ)
  · intro m n hm hmn
    have hm_real : (1 : ℝ) ≤ m := by exact_mod_cast hm
    have hmn_real : (m : ℝ) ≤ n := by exact_mod_cast hmn
    have hsub :
        (∫ x in (0 : ℝ)..(n : ℝ), Real.sinc x) -
          (∫ x in (0 : ℝ)..(m : ℝ), Real.sinc x) =
            ∫ x in (m : ℝ)..(n : ℝ), Real.sinc x := by
      exact intervalIntegral.integral_interval_sub_left
        (Real.continuous_sinc.intervalIntegrable _ _)
        (Real.continuous_sinc.intervalIntegrable _ _)
    rw [Real.dist_eq, ← Real.norm_eq_abs, hsub]
    exact norm_intervalIntegral_sinc_tail_le hm_real hmn_real

/-- The finite one-sided sinc windows are Cauchy over real heights. -/
...
/-- The damped sine function is integrable on the positive half-line. -/
theorem integrableOn_exp_neg_mul_sin (a : ℝ) (ha : 0 < a) :
    IntegrableOn (fun x : ℝ => Real.exp (-a * x) * Real.sin x) (Set.Ioi 0) := by
...
/-- Evaluation of the damped sine integral on the positive half-line. -/
...
/-- Principal-value Fourier inversion from source integrability and local
quotient integrability at the target point. -/
theorem fourierInvTrunc_tendsto_of_integrable_local_quotient
    [CompleteSpace E] {f : ℝ → E} (hf : Integrable f) {x R : ℝ} (hR : 0 < R)
    (hq : IntervalIntegrable
      (fun u : ℝ =>
        if u = 0 then 0 else (1 / (π * u) : ℂ) • (f (x - u) - f x))
    ...
    Filter.Tendsto (fun T : ℝ => fourierInvTrunc (𝓕 f) x T)
      Filter.atTop (nhds (f x)) := by
  exact fourierInvTrunc_tendsto_of_sinc_kernel (E := E) hf
    (sinc_kernel_tendsto_of_integrable_local_quotient (E := E) hf hR hq)

/-- Principal-value convergence once the only remaining mass/splitting identity
has been isolated. The hard analytic hypothesis is `hsplit`; the quotient
integrability and Riemann-Lebesgue cancellation close the rest. -/
theorem sinc_kernel_tendsto_of_pv_split
    {f : ℝ → E} {x : ℝ}
    (hq : Integrable
      (fun u : ℝ =>
        if u = 0 then 0 else (1 / (π * u) : ℂ) • (f (x - u) - f x)))
    ...
    Filter.Tendsto
      (fun T : ℝ =>
        (1 / (2 * π) : ℝ) •
          ∫ y : ℝ, (2 * T * Real.sinc (T * (x - y)) : ℂ) • f y)
      Filter.atTop (nhds (f x)) := by
  refine sinc_kernel_tendsto_of_sin_div_error (E := E) hq ?_
  filter_upwards [hsplit] with T hT
  rw [hT]
  congr 1
  exact integral_sin_div_kernel_sub_eq_sin_smul_quotient (E := E) f x T

/-- Principal-value Fourier inversion once the only remaining mass/splitting
identity has been isolated. -/
theorem fourierInvTrunc_tendsto_of_pv_split
    [CompleteSpace E] {f : ℝ → E} (hf : Integrable f) {x : ℝ}
    (hq : Integrable
      (fun u : ℝ =>
        if u = 0 then 0 else (1 / (π * u) : ℂ) • (f (x - u) - f x)))
    ...
    Filter.Tendsto (fun T : ℝ => fourierInvTrunc (𝓕 f) x T)
      Filter.atTop (nhds (f x)) := by
  exact fourierInvTrunc_tendsto_of_sinc_kernel (E := E) hf
    (sinc_kernel_tendsto_of_pv_split (E := E) hq hsplit)

/-- Truncated inverse Fourier integrals converge to the usual Fourier inversion
value when the Fourier transform is integrable. This is the finite-height
version of Mathlib's L1 inversion theorem. -/
theorem fourierInvTrunc_tendsto_of_integrable_fourier
    [CompleteSpace E] {f : ℝ → E} (hf : Integrable f) (hF : Integrable (𝓕 f))
    {x : ℝ} (hcont : ContinuousAt f x) :
    Filter.Tendsto (fun T : ℝ => fourierInvTrunc (𝓕 f) x T)
      Filter.atTop (nhds (f x)) := by
  let H : ℝ → E := fun t => Complex.exp (((t * x : ℝ) : ℂ) * I) • 𝓕 f (t / (2 * π))
  have hcomp : Integrable (fun t : ℝ => 𝓕 f (t / (2 * π))) := by
    have hR : (2 * π)⁻¹ ≠ (0 : ℝ) := by positivity
    simpa [div_eq_mul_inv] using hF.comp_mul_right' hR
  have hH : Integrable H := by
    refine hcomp.mono ?_ ?_
    · exact (by fun_prop : Continuous (fun t : ℝ => Complex.exp (((t * x : ℝ) : ℂ) * I)))
        |>.aestronglyMeasurable.smul hcomp.aestronglyMeasurable
    · filter_upwards with t
      rw [norm_smul, Complex.norm_exp_ofReal_mul_I, one_mul]
  have hlim_int : Filter.Tendsto (fun T : ℝ => ∫ t in (-T)..T, H t)
      Filter.atTop (nhds (∫ t : ℝ, H t)) := by
    exact MeasureTheory.intervalIntegral_tendsto_integral hH
      Filter.tendsto_neg_atTop_atBot Filter.tendsto_id
  have hlim := hlim_int.const_smul (1 / (2 * π) : ℝ)
  have htarget : (1 / (2 * π) : ℝ) • ∫ t : ℝ, H t = f x := by
    rw [← fourierInv_eq_integral_comp_div (F := 𝓕 f) x]
    exact hf.fourierInv_fourier_eq hF hcont
  rw [htarget] at hlim
  simpa [fourierInvTrunc, H] using hlim

/-- A finite-height inverse Laplace integral is a finite-height inverse Fourier
integral of the exponentially weighted source. The only analytic step left for
principal-value Laplace inversion is the convergence of these truncated Fourier
integrals. -/
...
/-- Principal-value bilateral Laplace inversion from integrability of the
exponentially weighted source and local quotient integrability at the target
point. -/
theorem laplaceInvLineTrunc_tendsto_laplaceTransformBilateral_of_integrable_local_quotient
    [CompleteSpace E]
    (sigma : ℝ) (f : ℝ → E) {x R : ℝ} (hR : 0 < R)
    (hg : Integrable
    ...
    (hq : IntervalIntegrable
      (fun u : ℝ =>
        if u = 0 then 0 else
          (1 / (π * u) : ℂ) •
            (Complex.exp (-((sigma : ℂ) * ((x - u : ℝ) : ℂ))) • f (x - u) -
              Complex.exp (-((sigma : ℂ) * (x : ℂ))) • f x))
      volume (-R) R) :
    Filter.Tendsto
      (fun T : ℝ => laplaceInvLineTrunc sigma (laplaceTransformBilateral f) x T)
      Filter.atTop (nhds (f x)) := by
  exact laplaceInvLineTrunc_tendsto_laplaceTransformBilateral_eq sigma f
    (fourierInvTrunc_tendsto_of_integrable_local_quotient (E := E)
      (f := fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) • f y)
      (x := x) (R := R) hg hR hq)

/-- Convert the exponential kernel on a positive real base into `x ^ s`. -/
...
/-- Principal-value Laplace inversion in the multiplication `x^s` form,
assuming convergence of the explicit Dirichlet-kernel integrals for the
exponentially weighted source. -/
theorem laplaceIntegralCpowTrunc_tendsto_of_dirichlet_kernel
    (sigma : ℝ) (f : ℝ → ℂ) {x : ℝ} (hx : 0 < x)
    ...
    (hdir : Filter.Tendsto
      (fun T : ℝ =>
        (1 / (2 * π) : ℝ) •
          ∫ y : ℝ,
            (if Real.log x - y = 0 then (2 * T : ℂ)
              else (2 * Real.sin (T * (Real.log x - y)) : ℂ) /
                ((Real.log x - y : ℝ) : ℂ)) •
  ...
  exact laplaceIntegralCpowTrunc_tendsto_of_fourierInvTrunc sigma f hx
    (fourierInvTrunc_tendsto_of_dirichlet_kernel (E := ℂ)
      (f := fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) * f y) hg hdir)

/-- Principal-value Laplace inversion in the multiplication `x^s` form,
assuming convergence of the scaled `sinc` kernel integrals for the exponentially
weighted source. -/
...
/-- Principal-value Laplace inversion in multiplication form from a sinc-kernel
representation whose error is an integrable sine integral. This is the sharpened
carry surface after the Riemann-Lebesgue cancellation has been discharged. -/
theorem laplaceIntegralCpowTrunc_tendsto_of_sinc_error
    (sigma : ℝ) (f : ℝ → ℂ) {x : ℝ} (hx : 0 < x)
    (hg : Integrable (fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) * f y))
    {q : ℝ → ℂ} (hq : Integrable q)
    (hrep : (fun T : ℝ =>
        (1 / (2 * π) : ℝ) •
          ∫ y : ℝ,
            (2 * T * Real.sinc (T * (Real.log x - y)) : ℂ) •
              (Complex.exp (-((sigma : ℂ) * (y : ℂ))) * f y)) =ᶠ[Filter.atTop]
        (fun T : ℝ =>
          Complex.exp (-((sigma : ℂ) * (Real.log x : ℂ))) * f (Real.log x) +
            ∫ u : ℝ, (Real.sin (T * u) : ℂ) • q u)) :
    Filter.Tendsto (fun T : ℝ => laplaceIntegralCpowTrunc f sigma x T)
      Filter.atTop (nhds (f (Real.log x))) := by
  exact laplaceIntegralCpowTrunc_tendsto_of_sinc_kernel sigma f hx hg
    (sinc_kernel_tendsto_of_integrable_sine_error (E := ℂ)
      (f := fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) * f y)
      (x := Real.log x) hq hrep)

/-- Principal-value Laplace inversion in multiplication form from an explicit
`sin (T * u) / (π * u)` representation whose error is an integrable sine
integral. This is the closest carry surface before the remaining Dirichlet/PV
mass identity. -/
theorem laplaceIntegralCpowTrunc_tendsto_of_sin_div_error
    (sigma : ℝ) (f : ℝ → ℂ) {x : ℝ} (hx : 0 < x)
    (hg : Integrable (fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) * f y))
    {q : ℝ → ℂ} (hq : Integrable q)
    (hrep : (fun T : ℝ =>
    ...
    Filter.Tendsto (fun T : ℝ => laplaceIntegralCpowTrunc f sigma x T)
      Filter.atTop (nhds (f (Real.log x))) := by
  exact laplaceIntegralCpowTrunc_tendsto_of_sinc_kernel sigma f hx hg
    (sinc_kernel_tendsto_of_sin_div_error (E := ℂ)
      (f := fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) * f y)
      (x := Real.log x) hq hrep)

/-- Principal-value Laplace inversion in multiplication form once the remaining
Dirichlet/PV mass-splitting identity has been isolated. -/
theorem laplaceIntegralCpowTrunc_tendsto_of_pv_split
    (sigma : ℝ) (f : ℝ → ℂ) {x : ℝ} (hx : 0 < x)
    (hg : Integrable (fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) * f y))
    (hq : Integrable
      (fun u : ℝ =>
    ...
    Filter.Tendsto (fun T : ℝ => laplaceIntegralCpowTrunc f sigma x T)
      Filter.atTop (nhds (f (Real.log x))) := by
  exact laplaceIntegralCpowTrunc_tendsto_of_sinc_kernel sigma f hx hg
    (sinc_kernel_tendsto_of_pv_split (E := ℂ)
      (f := fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) * f y)
      (x := Real.log x) hq hsplit)

/-- Principal-value Laplace inversion in multiplication form from explicit
finite-window mass, local-error, and tail-control inputs. The positive-radius
wrapper below discharges the mass input from the scalar sinc limit. -/
theorem laplaceIntegralCpowTrunc_tendsto_of_windowed_pv
    (sigma : ℝ) (f : ℝ → ℂ) {x R : ℝ} (hx : 0 < x)
    (hg : Integrable (fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) * f y))
    (hconst : ∀ᶠ T in Filter.atTop,
      IntervalIntegrable
        (fun u : ℝ =>
    ...
    Filter.Tendsto (fun T : ℝ => laplaceIntegralCpowTrunc f sigma x T)
      Filter.atTop (nhds (f (Real.log x))) := by
  exact laplaceIntegralCpowTrunc_tendsto_of_sinc_kernel sigma f hx hg
    (sinc_kernel_tendsto_of_windowed_pv (E := ℂ)
      (f := fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) * f y)
      (x := Real.log x) (R := R) hconst herrInt hmass herror htail)

/-- Principal-value Laplace inversion in multiplication form from finite-window
local-error and tail-control inputs, with scalar mass supplied by the sinc
limit. -/
theorem laplaceIntegralCpowTrunc_tendsto_of_windowed_pv_of_pos_radius
    ...
    Filter.Tendsto (fun T : ℝ => laplaceIntegralCpowTrunc f sigma x T)
      Filter.atTop (nhds (f (Real.log x))) := by
  exact laplaceIntegralCpowTrunc_tendsto_of_sinc_kernel sigma f hx hg
    (sinc_kernel_tendsto_of_windowed_pv_of_pos_radius (E := ℂ)
      (f := fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) * f y)
      (x := Real.log x) (R := R) hR herrInt herror htail)

/-- Principal-value Laplace inversion in multiplication form from local
quotient integrability and tail control. -/
theorem laplaceIntegralCpowTrunc_tendsto_of_windowed_pv_of_local_quotient
    (sigma : ℝ) (f : ℝ → ℂ) {x R : ℝ} (hx : 0 < x) (hR : 0 < R)
    (hg : Integrable (fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) * f y))
    (hq : IntervalIntegrable
      (fun u : ℝ =>
    ...
    Filter.Tendsto (fun T : ℝ => laplaceIntegralCpowTrunc f sigma x T)
      Filter.atTop (nhds (f (Real.log x))) := by
  exact laplaceIntegralCpowTrunc_tendsto_of_sinc_kernel sigma f hx hg
    (sinc_kernel_tendsto_of_windowed_pv_of_local_quotient (E := ℂ)
      (f := fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) * f y)
      (x := Real.log x) (R := R) hR hq htail)

/-- Principal-value Laplace inversion in multiplication form from source
integrability and local quotient integrability at `log x`. -/
...
/-- Finite-height Laplace inversion in the multiplication `x^s` form from
absolute integrability of the vertical line. The principal-value path should
use `laplaceIntegralCpowTrunc_tendsto_of_dirichlet_kernel` instead. -/
theorem laplaceIntegralCpowTrunc_tendsto_of_integrable_vertical
    (sigma : ℝ) (f : ℝ → ℂ) {x : ℝ} (hx : 0 < x)
    (hg : Integrable (fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) * f y))
    (hF : Integrable (fun t : ℝ => laplaceIntegral f ((sigma : ℂ) + (t : ℂ) * I)))
    (hcont : ContinuousAt f (Real.log x)) :
    Filter.Tendsto (fun T : ℝ => laplaceIntegralCpowTrunc f sigma x T)
      Filter.atTop (nhds (f (Real.log x))) := by
  let g : ℝ → ℂ := fun y => Complex.exp (-((sigma : ℂ) * (y : ℂ))) * f y
  have hF' : Integrable (fun t : ℝ =>
      laplaceTransformBilateral f ((sigma : ℂ) + (t : ℂ) * I)) := by
    simpa [laplaceIntegral_eq_laplaceTransformBilateral] using hF
  have hline_fourier : Integrable (fun t : ℝ => 𝓕 g (t / (2 * π))) := by
    simpa [g, laplaceTransformBilateral_eq_fourier, smul_eq_mul] using hF'
  have h2pi_real : (2 * π : ℝ) ≠ 0 := by positivity
  have hFg : Integrable (𝓕 g) := by
    have hcomp := hline_fourier.comp_mul_right' h2pi_real
    simpa [div_eq_mul_inv, mul_assoc, h2pi_real] using hcomp
  have hg_cont : ContinuousAt g (Real.log x) := by
    exact (Complex.continuous_exp.continuousAt.comp (by fun_prop)).mul hcont
  have hlim : Filter.Tendsto (fun T : ℝ => fourierInvTrunc (𝓕 g) (Real.log x) T)
      Filter.atTop
      (nhds (Complex.exp (-((sigma : ℂ) * (Real.log x : ℂ))) * f (Real.log x))) := by
    simpa [g] using
      fourierInvTrunc_tendsto_of_integrable_fourier (E := ℂ) hg hFg hg_cont
  exact laplaceIntegralCpowTrunc_tendsto_of_fourierInvTrunc sigma f hx hlim

/-- If `f` and its derivative are integrable, the Fourier transform of `f`
obeys the derivative-form estimate before division by the frequency. -/
theorem norm_smul_fourier_le_integral_norm_deriv
    {f : ℝ → E} (hf : Integrable f) (h'f : Differentiable ℝ f)
    (hf' : Integrable (deriv f)) (t : ℝ) :
    ‖(2 * (π : ℂ) * I * (t : ℂ)) • 𝓕 f t‖ ≤ ∫ x : ℝ, ‖deriv f x‖ := by
  have h_eq := congr_fun (Real.fourier_deriv hf h'f hf') t
  rw [← h_eq]
  have hbound : ‖𝓕 (deriv f) t‖ ≤ ∫ x : ℝ, ‖deriv f x‖ := by
    rw [show 𝓕 (deriv f) t = Fourier.fourierIntegral 𝐞 volume (deriv f) t by
      rw [Real.fourier_real_eq, Fourier.fourierIntegral_def]]
    exact Fourier.norm_fourierIntegral_le_integral_norm (E := E) 𝐞 volume (deriv f) t
  simpa using hbound

/-- If `f` and its derivative are integrable, the Fourier transform of `f`
has the standard `1 / |t|` decay away from zero. -/
theorem norm_fourier_le_integral_norm_deriv_div
    {f : ℝ → E} (hf : Integrable f) (h'f : Differentiable ℝ f)
    (hf' : Integrable (deriv f)) {t : ℝ} (ht : t ≠ 0) :
    ‖𝓕 f t‖ ≤ (∫ x : ℝ, ‖deriv f x‖) / (2 * π * |t|) := by
  have hmul := norm_smul_fourier_le_integral_norm_deriv
    (E := E) hf h'f hf' t
  have hscalar : ‖(2 * (π : ℂ) * I * (t : ℂ))‖ = 2 * π * |t| := by
    simp [Real.norm_eq_abs, abs_of_pos Real.pi_pos]
  rw [norm_smul, hscalar] at hmul
  have hpos : 0 < 2 * π * |t| := by positivity
  rw [mul_comm (2 * π * |t|) (‖𝓕 f t‖)] at hmul
  exact (le_div_iff₀ hpos).2 hmul

/-- Fourier derivative decay rewritten on a vertical bilateral-Laplace line. -/
theorem norm_laplaceTransformBilateral_le_integral_norm_deriv_div
    (sigma : ℝ) {f : ℝ → E}
    (hg : Integrable (fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) • f y))
    (h'g : Differentiable ℝ (fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) • f y))
    (hg' : Integrable
      (deriv (fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) • f y)))
    {t : ℝ} (ht : t ≠ 0) :
    ‖laplaceTransformBilateral f ((sigma : ℂ) + (t : ℂ) * I)‖ ≤
      (∫ y : ℝ, ‖deriv
        (fun z : ℝ => Complex.exp (-((sigma : ℂ) * (z : ℂ))) • f z) y‖) / |t| := by
  let g : ℝ → E := fun y => Complex.exp (-((sigma : ℂ) * (y : ℂ))) • f y
  have ht' : t / (2 * π) ≠ 0 := by
    exact div_ne_zero ht (by positivity)
  have hFourier := norm_fourier_le_integral_norm_deriv_div
    (E := E) (f := g) hg h'g hg' ht'
  have hdenom : 2 * π * |t / (2 * π)| = |t| := by
    rw [abs_div]
    rw [abs_of_pos (by positivity : 0 < 2 * π)]
    field_simp [Real.pi_ne_zero]
  have hline :
      laplaceTransformBilateral f ((sigma : ℂ) + (t : ℂ) * I) = 𝓕 g (t / (2 * π)) := by
    simpa [g] using
      laplaceTransformBilateral_eq_fourier (E := E) f ((sigma : ℂ) + (t : ℂ) * I)
  rw [hline]
  simpa [g, hdenom] using hFourier

/-- Multiplication-form version of vertical bilateral-Laplace `1 / |t|`
decay for complex-valued functions. -/
theorem norm_laplaceIntegral_le_integral_norm_deriv_div
    (sigma : ℝ) {f : ℝ → ℂ}
    (hg : Integrable (fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) * f y))
    (h'g : Differentiable ℝ (fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) * f y))
    (hg' : Integrable
      (deriv (fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) * f y)))
    {t : ℝ} (ht : t ≠ 0) :
    ‖laplaceIntegral f ((sigma : ℂ) + (t : ℂ) * I)‖ ≤
      (∫ y : ℝ, ‖deriv
        (fun z : ℝ => Complex.exp (-((sigma : ℂ) * (z : ℂ))) * f z) y‖) / |t| := by
  have h := norm_laplaceTransformBilateral_le_integral_norm_deriv_div
    (E := ℂ) sigma (f := f) (t := t) (by simpa [smul_eq_mul] using hg)
    (by simpa [smul_eq_mul] using h'g) (by simpa [smul_eq_mul] using hg') ht
  simpa [laplaceIntegral_eq_laplaceTransformBilateral, smul_eq_mul] using h

variable [CompleteSpace E]

/-- Bilateral Laplace inversion from Fourier inversion, with the Fourier hypotheses rewritten on
the vertical Laplace line. -/
theorem laplaceInvLine_laplaceTransformBilateral_eq
    (sigma : ℝ) (f : ℝ → E) {x : ℝ}
    (hf : Integrable (fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) • f y))
    (hF : Integrable (fun t : ℝ => laplaceTransformBilateral f ((sigma : ℂ) + (t : ℂ) * I)))
    (hfx : ContinuousAt f x) :
    laplaceInvLine sigma (laplaceTransformBilateral f) x = f x := by
  let g : ℝ → E := fun y => Complex.exp (-((sigma : ℂ) * (y : ℂ))) • f y
  have hg : Integrable g := hf
  have hline_fourier : Integrable (fun t : ℝ => 𝓕 g (t / (2 * π))) := by
    simpa [g, laplaceTransformBilateral_eq_fourier] using hF
  have h2pi_real : (2 * π : ℝ) ≠ 0 := by positivity
  have hFg : Integrable (𝓕 g) := by
    have hcomp := hline_fourier.comp_mul_right' h2pi_real
    simpa [div_eq_mul_inv, mul_assoc, h2pi_real] using hcomp
  have hg_cont : ContinuousAt g x := by
    exact (Complex.continuous_exp.continuousAt.comp (by fun_prop)).smul hfx
  have hFourier := hg.fourierInv_fourier_eq hFg hg_cont
  unfold laplaceInvLine
  simp_rw [laplaceTransformBilateral_eq_fourier]
  simp only [one_div, mul_inv_rev, add_re, ofReal_re, mul_re, I_re, mul_zero, ofReal_im,
    I_im, mul_one, sub_self, add_zero, neg_mul, add_im, mul_im, zero_add]
  rw [show (π⁻¹ * 2⁻¹ : ℝ) = (1 / (2 * π) : ℝ) by ring]
  ...
  have hsplit :
      (∫ t : ℝ, Complex.exp (((sigma : ℂ) + (t : ℂ) * I) * (x : ℂ)) •
          𝓕 g (t / (2 * π))) =
        Complex.exp ((sigma : ℂ) * (x : ℂ)) •
          ∫ t : ℝ, Complex.exp (((t * x : ℝ) : ℂ) * I) • 𝓕 g (t / (2 * π)) := by
    rw [← integral_smul]
    congr with t
    rw [← smul_assoc]
    congr 1
    ...
    (1 / (2 * π) : ℝ) •
        ...
        = (1 / (2 * π) : ℝ) •
            (Complex.exp ((sigma : ℂ) * (x : ℂ)) •
              ∫ t : ℝ, Complex.exp (((t * x : ℝ) : ℂ) * I) • 𝓕 g (t / (2 * π))) := by
          rw [hsplit]
    _ = Complex.exp ((sigma : ℂ) * (x : ℂ)) •
          ((1 / (2 * π) : ℝ) •
            ∫ t : ℝ, Complex.exp (((t * x : ℝ) : ℂ) * I) • 𝓕 g (t / (2 * π))) := by
          rw [SMulCommClass.smul_comm (M := ℝ) (N := ℂ) (α := E)]
    _ = Complex.exp ((sigma : ℂ) * (x : ℂ)) • 𝓕⁻ (𝓕 g) x := by
          rw [← fourierInv_eq_integral_comp_div (F := 𝓕 g) x]
    _ = Complex.exp ((sigma : ℂ) * (x : ℂ)) • g x := by
          rw [hFourier]
    _ = f x := by
          simp [g, ← smul_assoc, ← Complex.exp_add]

/-- Complex-valued bilateral Laplace inversion in the power-kernel form used by
Perron and explicit-formula arguments. -/
theorem laplaceTransformBilateral_inversion_cpow_of_pos
    (sigma : ℝ) (f : ℝ → ℂ) {x : ℝ} (hx : 0 < x)
    (hf : Integrable (fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) * f y))
    (hF : Integrable (fun t : ℝ => laplaceTransformBilateral f ((sigma : ℂ) + (t : ℂ) * I)))
    (hfx : ContinuousAt f (Real.log x)) :
    f (Real.log x) =
      (1 / (2 * (π : ℂ))) *
        ∫ t : ℝ, laplaceTransformBilateral f ((sigma : ℂ) + (t : ℂ) * I) *
          (x : ℂ) ^ ((sigma : ℂ) + (t : ℂ) * I) := by
  have h := laplaceInvLine_laplaceTransformBilateral_eq (E := ℂ)
    sigma f (x := Real.log x) hf hF hfx
  rw [← h]
  unfold laplaceInvLine
  simp_rw [smul_eq_mul]
  rw [RCLike.real_smul_eq_coe_mul]
  ...
  · congr with t
    rw [mul_comm]
    congr 1
    exact Complex.exp_mul_log_of_pos_eq_cpow hx ((sigma : ℂ) + (t : ℂ) * I)

/-- Complex-valued bilateral Laplace inversion in the multiplication form
`∫ y, f y * exp (-s * y)`. -/
theorem laplaceIntegral_inversion_cpow_of_pos
    (sigma : ℝ) (f : ℝ → ℂ) {x : ℝ} (hx : 0 < x)
    (hf : Integrable (fun y : ℝ => Complex.exp (-((sigma : ℂ) * (y : ℂ))) * f y))
    (hF : Integrable (fun t : ℝ => laplaceIntegral f ((sigma : ℂ) + (t : ℂ) * I)))
    (hfx : ContinuousAt f (Real.log x)) :
    f (Real.log x) =
      (1 / (2 * (π : ℂ))) *
        ∫ t : ℝ, laplaceIntegral f ((sigma : ℂ) + (t : ℂ) * I) *
          (x : ℂ) ^ ((sigma : ℂ) + (t : ℂ) * I) := by
  have hF' : Integrable (fun t : ℝ =>
      laplaceTransformBilateral f ((sigma : ℂ) + (t : ℂ) * I)) := by
    simpa [laplaceIntegral_eq_laplaceTransformBilateral] using hF
  have h := laplaceTransformBilateral_inversion_cpow_of_pos sigma f hx hf hF' hfx
  rw [h]
  congr with t
  rw [laplaceIntegral_eq_laplaceTransformBilateral]
