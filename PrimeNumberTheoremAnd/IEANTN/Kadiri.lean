...
/-! ### Weighted Hadamard bridge

The xi Hadamard product is indexed by the divisor of `riemannXi`, so it repeats zeros by
multiplicity. The Kadiri-facing zero packet is therefore `riemannZeta.zeroes_sum`, not a raw
`tsum` over the plain `zeroes_rect` set. -/
...
/--
The cosine factor in the zeta functional equation is nonzero when the real
part lies strictly between `1` and `2`.
-/
private lemma kadiri_eq12_cos_pi_mul_div_two_ne_zero_of_re_mem_Ioo_one_two
    {w : ℂ} (hwre1 : 1 < w.re) (hwre2 : w.re < 2) :
    Complex.cos ((Real.pi : ℂ) * w / 2) ≠ 0 := by
  rw [Complex.cos_ne_zero_iff]
  intro k hk
  have hre_eq : (((Real.pi : ℂ) * w / 2).re : ℝ) =
      (((2 * (k : ℂ) + 1) * (Real.pi : ℂ) / 2).re : ℝ) := by
    rw [hk]
  have hleft : (((Real.pi : ℂ) * w / 2).re : ℝ) = Real.pi * w.re / 2 := by
    simp [div_eq_mul_inv, mul_assoc]
  have hright : ((((2 * (k : ℂ) + 1) * (Real.pi : ℂ) / 2).re : ℝ)) =
      ((2 * k + 1 : ℤ) : ℝ) * Real.pi / 2 := by
    simp [div_eq_mul_inv, mul_assoc]
  have hwre_eq : w.re = ((2 * k + 1 : ℤ) : ℝ) := by
    have hmul : Real.pi * w.re / 2 =
        ((2 * k + 1 : ℤ) : ℝ) * Real.pi / 2 := by
      simpa [hleft, hright] using hre_eq
    have hmul2 : Real.pi * w.re =
        ((2 * k + 1 : ℤ) : ℝ) * Real.pi := by
      nlinarith
    nlinarith [Real.pi_pos]
  have hk_cases : k ≤ 0 ∨ 1 ≤ k := by omega
  rcases hk_cases with hk_nonpos | hk_pos
  · have hodd_le : ((2 * k + 1 : ℤ) : ℝ) ≤ 1 := by
      exact_mod_cast (by omega : 2 * k + 1 ≤ 1)
    linarith
  · have hodd_ge : (3 : ℝ) ≤ ((2 * k + 1 : ℤ) : ℝ) := by
      exact_mod_cast (by omega : 3 ≤ 2 * k + 1)
    linarith

/--
There are no zeta zeros on the real segment `-1 < Re z ≤ 0`.
The endpoint `0` is handled by `riemannZeta_zero`; the open segment follows
from the functional equation and nonvanishing on `Re s ≥ 1`.
-/
theorem riemannZeta_ne_zero_of_neg_one_lt_re_nonpos_im_zero {z : ℂ}
    (hneg : -1 < z.re) (hre : z.re ≤ 0) (him : z.im = 0) :
    riemannZeta z ≠ 0 := by
  by_cases hz0 : z = 0
  · subst z
    rw [riemannZeta_zero]
    norm_num
  · let w : ℂ := 1 - z
    have hzre_ne_zero : z.re ≠ 0 := by
      intro hzre
      apply hz0
      exact Complex.ext_iff.2 ⟨by simpa using hzre, by simpa using him⟩
    have hzre_lt_zero : z.re < 0 := lt_of_le_of_ne hre hzre_ne_zero
    have hw_re_gt : 1 < w.re := by
      change 1 < ((1 : ℂ) - z).re
      simp
      linarith
    have hw_re_lt : w.re < 2 := by
      change (((1 : ℂ) - z).re : ℝ) < 2
      simp
      linarith
    have hzeta_w : riemannZeta w ≠ 0 :=
      riemannZeta_ne_zero_of_one_le_re hw_re_gt.le
    have hw_neg_nat : ∀ n : ℕ, w ≠ -↑n := by
      intro n hn
      have hrew : w.re = (-↑n : ℂ).re := congrArg Complex.re hn
      have hn_nonpos : ((-↑n : ℂ).re : ℝ) ≤ 0 := by simp
      linarith
    have hw_ne_one : w ≠ 1 := by
      intro h
      have hrew : w.re = (1 : ℂ).re := congrArg Complex.re h
      change (((1 : ℂ) - z).re : ℝ) = 1 at hrew
      simp at hrew
      linarith
    ...
    have hcos : Complex.cos (↑Real.pi * w / 2) ≠ 0 :=
      kadiri_eq12_cos_pi_mul_div_two_ne_zero_of_re_mem_Ioo_one_two hw_re_gt hw_re_lt
...
/--
The real left part of the eq-12 rectangle contains no zeta zeros. Since
`a < 1`, points with `Re z ≤ 0` in the rectangle have `-1 < Re z`.
-/
theorem kadiri_eq12_left_real_zeta_ne_of_rectangle
    {a T : ℝ} (ha : 0 < a) (ha1 : a < 1) (hT : 0 < T) :
      ∀ z ∈ Rectangle (((-a : ℝ) : ℂ) - (T : ℂ) * I)
          (((1 + a : ℝ) : ℂ) + (T : ℂ) * I),
        z.re ≤ 0 → z.im = 0 → riemannZeta z ≠ 0 := by
  intro z hzrect hzre_nonpos hzim
  have hzre_le : (((-a : ℝ) : ℂ) - (T : ℂ) * I).re ≤
      (((1 + a : ℝ) : ℂ) + (T : ℂ) * I).re := by
    simp
    linarith
  have hzim_le : (((-a : ℝ) : ℂ) - (T : ℂ) * I).im ≤
    ...
    linarith
  have hzcoords := (mem_Rect hzre_le hzim_le z).mp hzrect
  rcases hzcoords with ⟨hre_low, _hre_high, _him_low, _him_high⟩
  have hzneg : -1 < z.re := by
    have hre_low' : -a ≤ z.re := by
      simpa using hre_low
    linarith
  exact riemannZeta_ne_zero_of_neg_one_lt_re_nonpos_im_zero hzneg hzre_nonpos hzim

/--
Inside the eq-12 rectangle with `a < 1`, zeta is nonzero away from the closed
candidate support `{1} ∪ zeroes_rect`.
-/
theorem kadiri_eq12_zeta_ne_off_candidate
    {a T : ℝ} (ha : 0 < a) (hT : 0 < T) (ha1 : a < 1) :
      ∀ z ∈ Rectangle (((-a : ℝ) : ℂ) - (T : ℂ) * I)
          (((1 + a : ℝ) : ℂ) + (T : ℂ) * I),
        z ∉ ({(1 : ℂ)} ∪ riemannZeta.zeroes_rect (.Ioo 0 1) (.Icc (-T) T)) →
          z ≠ 1 ∧ riemannZeta z ≠ 0 :=
  kadiri_eq12_zeta_ne_off_candidate_of_left_real_ne ha hT
    (kadiri_eq12_left_real_zeta_ne_of_rectangle ha ha1 hT)
...
/-- A bounded simple-pole principal part gives an at-most-simple order bound. -/
theorem meromorphicOrderAt_ge_neg_one_of_isBigO_sub_simple_pole {f : ℂ → ℂ} {p A : ℂ}
    (hmero : MeromorphicAt f p)
    (near_p : (f - fun z ↦ A / (z - p)) =O[𝓝[≠] p] (1 : ℂ → ℂ)) :
    ((-1 : ℤ) : WithTop ℤ) ≤ meromorphicOrderAt f p := by
  have hsub_tendsto : Tendsto (fun z : ℂ ↦ z - p) (𝓝[≠] p) (𝓝 0) := by
    simpa using
      ((continuous_id.sub continuous_const).continuousAt.continuousWithinAt.tendsto :
        Tendsto (fun z : ℂ ↦ z - p) (𝓝[≠] p) (𝓝 (p - p)))
  ...
  have htarget : Tendsto (fun z : ℂ ↦ A + (z - p) * ((f - fun z ↦ A / (z - p)) z))
      (𝓝[≠] p) (𝓝 A) := by
    simpa using tendsto_const_nhds.add hrem_zero
  have hmul_tendsto : Tendsto (fun z : ℂ ↦ (z - p) * f z) (𝓝[≠] p) (𝓝 A) := by
    refine htarget.congr' ?_
    filter_upwards [self_mem_nhdsWithin] with z hz
    have hzp : z - p ≠ 0 := sub_ne_zero.mpr hz
    simp [Pi.sub_apply, div_eq_mul_inv]
    field_simp [hzp]
    ring
  have hmul_mero : MeromorphicAt (fun z : ℂ ↦ (z - p) * f z) p := by
    change MeromorphicAt ((fun z : ℂ ↦ z - p) * f) p
    exact (by fun_prop : MeromorphicAt (fun z : ℂ ↦ z - p) p).mul hmero
  have hmul_nonneg :
      (0 : WithTop ℤ) ≤ meromorphicOrderAt (fun z : ℂ ↦ (z - p) * f z) p := by
    exact (tendsto_nhds_iff_meromorphicOrderAt_nonneg hmul_mero).1 ⟨A, hmul_tendsto⟩
  change (0 : WithTop ℤ) ≤ meromorphicOrderAt ((fun z : ℂ ↦ z - p) * f) p at hmul_nonneg
  rw [meromorphicOrderAt_mul (by fun_prop : MeromorphicAt (fun z : ℂ ↦ z - p) p)
    hmero, meromorphicOrderAt_id_sub_const] at hmul_nonneg
  cases hord : meromorphicOrderAt f p with
  | top => simp
  | coe n =>
      rw [hord] at hmul_nonneg
      have hn : (0 : ℤ) ≤ 1 + n := WithTop.coe_le_coe.mp (by simpa using hmul_nonneg)
...
-/
theorem kadiri_eq12_one_order_ge_neg_one_of_meromorphicAt {Φ : ℂ → ℂ}
    (hΦ : AnalyticAt ℂ Φ (-1))
    (hmero_one : MeromorphicAt (fun s ↦ (-logDeriv riemannZeta s) * Φ (-s)) (1 : ℂ)) :
    ((-1 : ℤ) : WithTop ℤ) ≤
      meromorphicOrderAt (fun s ↦ (-logDeriv riemannZeta s) * Φ (-s)) (1 : ℂ) := by
  let f : ℂ → ℂ := fun s ↦ -logDeriv riemannZeta s
  let g : ℂ → ℂ := fun s ↦ Φ (-s)
  have hΦ_comp : AnalyticAt ℂ g (1 : ℂ) := by
    have hneg : AnalyticAt ℂ (fun s : ℂ ↦ -s) (1 : ℂ) := by
  ...
  have hnear :
      (f * g - fun z : ℂ ↦ 1 * g (1 : ℂ) * (z - 1)⁻¹) =O[𝓝[≠] (1 : ℂ)]
        (1 : ℂ → ℂ) :=
    ResidueMult g_holc hU f_near_p
  refine meromorphicOrderAt_ge_neg_one_of_isBigO_sub_simple_pole
    (f := fun s ↦ (-logDeriv riemannZeta s) * Φ (-s)) (p := (1 : ℂ)) (A := Φ (-1))
    hmero_one ?_
  convert hnear using 1
  ext z
  simp [f, g, div_eq_mul_inv]
...
/--
The closed-height candidate pole set has only at-most-simple Kadiri integrand
poles under the off-height conversion.
-/
theorem kadiri_eq12_candidate_order_ge_neg_one_of_off_height
    {Φ : ℂ → ℂ} {T : ℝ}
    (hoff : ∀ rho : NontrivialZeros, (rho : ℂ).im ≠ T)
    (hΦ_one : AnalyticAt ℂ Φ (-1))
    (hΦ_zero : ∀ ρ : riemannZeta.zeroes_rect (.Ioo 0 1) (.Ioo (-T) T),
    ...
    ∀ z ∈ ({(1 : ℂ)} ∪ riemannZeta.zeroes_rect (.Ioo 0 1) (.Icc (-T) T)),
      ((-1 : ℤ) : WithTop ℤ) ≤
        meromorphicOrderAt (fun s ↦ (-logDeriv riemannZeta s) * Φ (-s)) z := by
  intro z hz
  rcases hz with hz1 | hzzero
  · subst z
    exact kadiri_eq12_one_order_ge_neg_one_of_meromorphicAt hΦ_one hmero_one
  · exact kadiri_eq12_integrand_order_ge_neg_one_closed_height_zero_of_off_height
      hoff hΦ_zero ⟨z, hzzero⟩
...
/--
CH2 residue-sum bridge for the Kadiri equation (12) rectangle, using equality
only after intersecting with the actual pole set. This admits cancellation of a
candidate pole by `Φ`.
-/
theorem kadiri_eq12_rectangleIntegral_eq_residue_packet_of_pole_inter
    {Φ : ℂ → ℂ} {a T : ℝ} (ha : 0 < a) (hT : 0 < T)
    (hΦ_one : AnalyticAt ℂ Φ (-1))
    (hΦ_zero : ∀ ρ : riemannZeta.zeroes_rect (.Ioo 0 1) (.Ioo (-T) T),
      AnalyticAt ℂ Φ (-(ρ : ℂ)))
  ...
  let C : Set ℂ := {(1 : ℂ)} ∪ riemannZeta.zeroes_rect (.Ioo 0 1) (.Ioo (-T) T)
  have hzle : z.re ≤ w.re := by
    dsimp [z, w]
    simp
    linarith
  have hwle : z.im ≤ w.im := by
    ...
    simp
    linarith
  have h_inter : R ∩ P = C ∩ P := by
    simpa [R, P, C, F, z, w] using hpoles_inter_eq
  have hpoles_fin : (R ∩ P).Finite := by
    rw [h_inter]
    exact (kadiri_eq12_candidate_residue_set_finite T).subset (by
      intro u hu
      exact hu.1)
  have hsum_actual_eq_candidate : CH2.sumResiduesIn F (R ∩ P) = CH2.sumResiduesIn F C := by
    refine CH2.sumResiduesIn_inter_eq_of_set_eq
      (F := F) (Rn := R) (S2 := C) (P := P) h_inter ?_
    intro s hsC hsNotP
    have hsR : s ∈ R := by
      exact kadiri_eq12_candidate_residue_set_subset_rectangle (a := a) (T := T) ha hT hsC
    have hs_nonneg : (0 : WithTop ℤ) ≤ meromorphicOrderAt F s :=
      le_of_not_gt (by simpa [P] using hsNotP)
    exact CH2.residue_eq_zero_of_not_pole_of_meromorphicAt
      (hmero s (by simpa [R, F, z, w] using hsR)) hs_nonneg
  calc
    RectangleIntegral' F z w =
        CH2.sumResiduesIn F (R ∩ P) := by
      exact CH2.RectangleIntegral'_eq_sumResiduesIn hzle hwle
        (by simpa [F, R, z, w] using hmero)
        (by simpa [F, P, z, w] using hno_boundary)
        hpoles_fin
        (by simpa [F, R, z, w] using hsimple)
    _ = CH2.sumResiduesIn F C := hsum_actual_eq_candidate
    _ = Φ (-1) - riemannZeta.zeroes_sum (.Ioo 0 1) (.Ioo (-T) T)
        (fun ρ ↦ Φ (-ρ)) := by
      simpa [F, C] using kadiri_eq12_sumResiduesIn_candidate_eq_of_analytic
        (Φ := Φ) (T := T) hΦ_one hΦ_zero

/--
The closed-height pole-set identity reduces `HasSimplePolesOn` to the local
order bounds at `1` and at the non-trivial zeros.
-/
theorem kadiri_eq12_hasSimplePolesOn_of_closed_pole_set
    {Φ : ℂ → ℂ} {a T : ℝ} (ha : 0 < a) (hT : 0 < T)
    (hoff : ∀ rho : NontrivialZeros, (rho : ℂ).im ≠ T)
    (hΦ_one : AnalyticAt ℂ Φ (-1))
    (hΦ_zero : ∀ ρ : riemannZeta.zeroes_rect (.Ioo 0 1) (.Ioo (-T) T),
    ...
    CH2.HasSimplePolesOn
      ...
      (Rectangle (((-a : ℝ) : ℂ) - (T : ℂ) * I)
        (((1 + a : ℝ) : ℂ) + (T : ℂ) * I)) := by
  intro z hzrect
  by_cases hzneg : meromorphicOrderAt (fun s ↦ (-logDeriv riemannZeta s) * Φ (-s)) z < 0
  · have hcandidate :
    ...
    have h1_rect : (1 : ℂ) ∈
      ...
      constructor
      · change (1 : ℝ) ∈ Set.uIcc (-a) (1 + a)
        rw [Set.uIcc_of_le (by linarith [ha])]
        exact ⟨by linarith [ha], by linarith [ha]⟩
      · change (0 : ℝ) ∈ Set.uIcc (-T) T
        rw [Set.uIcc_of_le (by linarith [hT])]
        exact ⟨by linarith [hT], by linarith [hT]⟩
    have hmero_one :
        MeromorphicAt (fun s ↦ (-logDeriv riemannZeta s) * Φ (-s)) (1 : ℂ) :=
      hmero (1 : ℂ) h1_rect
    exact kadiri_eq12_candidate_order_ge_neg_one_of_off_height
      hoff hΦ_one hΦ_zero hmero_one z hcandidate
  · have hnonneg :
        (0 : WithTop ℤ) ≤
          meromorphicOrderAt (fun s ↦ (-logDeriv riemannZeta s) * Φ (-s)) z :=
      le_of_not_gt hzneg
    exact le_trans (WithTop.coe_le_coe.mpr (by norm_num : (-1 : ℤ) ≤ 0)) hnonneg

/--
Closed-height `HasSimplePolesOn` from equality after intersecting with the
actual pole set.
-/
theorem kadiri_eq12_hasSimplePolesOn_of_closed_pole_inter
    {Φ : ℂ → ℂ} {a T : ℝ} (ha : 0 < a) (hT : 0 < T)
    (hoff : ∀ rho : NontrivialZeros, (rho : ℂ).im ≠ T)
    (hΦ_one : AnalyticAt ℂ Φ (-1))
    (hΦ_zero : ∀ ρ : riemannZeta.zeroes_rect (.Ioo 0 1) (.Ioo (-T) T),
  ...
  · have hcandidate :
        z ∈ ({(1 : ℂ)} ∪ riemannZeta.zeroes_rect (.Ioo 0 1) (.Icc (-T) T)) := by
      have hz_candidate_pole :
          z ∈ ({(1 : ℂ)} ∪ riemannZeta.zeroes_rect (.Ioo 0 1) (.Icc (-T) T)) ∩
            {u | meromorphicOrderAt
              (fun s ↦ (-logDeriv riemannZeta s) * Φ (-s)) u < 0} := by
        rw [← hpoles_inter_eq_closed]
        exact ⟨hzrect, hzneg⟩
      exact hz_candidate_pole.1
    have h1_rect : (1 : ℂ) ∈
        Rectangle (((-a : ℝ) : ℂ) - (T : ℂ) * I)
          (((1 + a : ℝ) : ℂ) + (T : ℂ) * I) := by
      simp only [Rectangle, Complex.ofReal_re, Complex.ofReal_im, Complex.sub_re,
...
theorem kadiri_eq12_rectangleIntegral_eq_residue_packet_of_closed_pole_set
  ...
  refine kadiri_eq12_rectangleIntegral_eq_residue_packet_of_pole_set
    ...
    (hmero := hmero) ?_
    (hsimple := kadiri_eq12_hasSimplePolesOn_of_closed_pole_set
      (ha := ha) (hT := hT) (hoff := hoff) (hΦ_one := hΦ_one)
      (hΦ_zero := hΦ_zero) (hmero := hmero) (hpoles_eq_closed := hpoles_eq_closed))
    ?_
...
/--
Closed-height residue bridge using equality after intersecting with actual
poles. The off-height hypothesis removes the two horizontal-border heights.
-/
theorem kadiri_eq12_rectangleIntegral_eq_residue_packet_of_closed_pole_inter
    {Φ : ℂ → ℂ} {a T : ℝ} (ha : 0 < a) (hT : 0 < T)
    (hoff : ∀ rho : NontrivialZeros, (rho : ℂ).im ≠ T)
    (hΦ_one : AnalyticAt ℂ Φ (-1))
    (hΦ_zero : ∀ ρ : riemannZeta.zeroes_rect (.Ioo 0 1) (.Ioo (-T) T),
    ...
    RectangleIntegral' (fun s ↦ (-logDeriv riemannZeta s) * Φ (-s))
      ...
      Φ (-1) - riemannZeta.zeroes_sum (.Ioo 0 1) (.Ioo (-T) T)
        (fun ρ ↦ Φ (-ρ)) := by
  refine kadiri_eq12_rectangleIntegral_eq_residue_packet_of_pole_inter
    (ha := ha) (hT := hT) (hΦ_one := hΦ_one) (hΦ_zero := hΦ_zero)
    (hmero := hmero) ?_
    (hsimple := kadiri_eq12_hasSimplePolesOn_of_closed_pole_inter
      (ha := ha) (hT := hT) (hoff := hoff) (hΦ_one := hΦ_one)
...
/--
Closed-height residue bridge from full rectangle analyticity of `Φ`, rectangle
meromorphicity of the integrand, and the eq-12 strip condition `a < 1`.
-/
theorem kadiri_eq12_rectangleIntegral_eq_residue_packet_of_phi_rect
  ...
  exact kadiri_eq12_rectangleIntegral_eq_residue_packet_of_zeta_ne_off_candidate
    (ha := ha) (hT := hT) (hoff := hoff) (hΦ_one := hΦ_one)
    (hΦ_zero := hΦ_zero) (hΦ_rect := hΦ_rect) (hmero := hmero)
...
package. The remaining mathematical content is the package of analyticity,
meromorphicity, and the closed-height pole-set identity for `(-ζ'/ζ) * Φ(-s)`.
-/
...
/--
Assembly form of Kadiri equation (12) from the closed-height analytic pole
package, with the support identity only after intersecting with actual poles.
-/
theorem kadiri_thm_3_1_q1_eq_12_of_closed_pole_inter_package
    {φ : ℝ → ℂ} {a T : ℝ} (ha : 0 < a) (hT : 0 < T)
    (hoff : ∀ rho : NontrivialZeros, (rho : ℂ).im ≠ T)
    (Φ : ℂ → ℂ)
    (hΦ : Φ = fun s ↦ ∫ y, φ y * exp (-s * (y : ℂ)) ∂volume)
  ...
  exact kadiri_thm_3_1_q1_eq_12_from_rectangleIntegral
    (ha := ha) (hT := hT) (Φ := Φ) hΦ
    (kadiri_eq12_rectangleIntegral_eq_residue_packet_of_closed_pole_inter
      (ha := ha) (hT := hT) (hoff := hoff) (hΦ_one := hΦ_one)
      (hΦ_zero := hΦ_zero) (hmero := hmero)
      (hpoles_inter_eq_closed := hpoles_inter_eq_closed))
...
analyticity of `Φ` and rectangle meromorphicity of the Kadiri integrand.
-/
theorem kadiri_thm_3_1_q1_eq_12_rectangular_residue_decomposition_of_phi_rect
    {φ : ℝ → ℂ} (_hφ : ContDiff ℝ 1 φ)
    {b : ℝ} (_hb : 0 < b)
    (_hφ_decay : (fun x : ℝ ↦ φ x * exp ((x : ℂ) / 2))
        =O[Filter.cocompact ℝ] fun x : ℝ ↦ Real.exp (-(1/2 + b) * |x|))
...
the pole at `1`, identify the enclosed non-trivial zeros, justify the rectangle
side decomposition, and convert the resulting finite residue contribution into
`riemannZeta.zeroes_sum`.
-/
theorem kadiri_thm_3_1_q1_eq_12_rectangular_residue_decomposition
    {φ : ℝ → ℂ} (_hφ : ContDiff ℝ 1 φ)
    {b : ℝ} (_hb : 0 < b)
    (_hφ_decay : (fun x : ℝ ↦ φ x * exp ((x : ℂ) / 2))
        =O[Filter.cocompact ℝ] fun x : ℝ ↦ Real.exp (-(1/2 + b) * |x|))
