...
theorem zeroes_sum_eq_finset_of_finite {α : Type*} [RCLike α]
    (I J : Set ℝ) (f : ℂ → α)
...
theorem u6aFTNearbyWindow_finite (t : ℝ) : (u6aFTNearbyWindow t).Finite := by
  rw [u6aFTNearbyWindow, riemannZeta.zeroes_rect_eq]
...
lemma riemannZeta_order_pos_of_zero_ne_one {s : ℂ} (hs : s ≠ 1)
  ...
  have han := riemannZeta_analyticOn_compl_one s
    (by simpa [Set.mem_compl_iff] using hs)
  have horder_ne_top : meromorphicOrderAt riemannZeta s ≠ ⊤ := by
    exact (meromorphicOrderAt_ne_top_iff_eventually_ne_zero han.meromorphicAt).2
      (by
        ...
        have heventually_compl_one : ∀ᶠ z in nhdsWithin s ({s}ᶜ),
            z ∈ ({1} : Set ℂ)ᶜ :=
          nhdsWithin_le_nhds (isOpen_compl_singleton.mem_nhds hmem_compl_one)
        filter_upwards [hnot_zeroes, heventually_compl_one] with z hznot hz_compl_one hz
...
/-- A non-real zeta zero lies strictly inside the critical strip. -/
theorem riemannZeta_zero_re_mem_Ioo_of_im_ne_zero' {ρ : ℂ}
  ...
  · by_contra h
    ...
    have hcompleted : completedRiemannZeta ρ = 0 := by
      ...
      · exact h2
      · exact absurd h2 hΓℝ
    set s : ℂ := 1 - ρ with hsdef
    have hFE : completedRiemannZeta s = 0 := by
      rw [hsdef, completedRiemannZeta_one_sub]
      exact hcompleted
    have hs1 : s ≠ 1 := by
      intro hc
    ...
    exact riemannZeta_ne_zero_of_one_le_re
      (by rw [hsdef, Complex.sub_re, Complex.one_re]; linarith) hζs
  · by_contra h
    push Not at h
...
theorem differentiable_u6aShiftedZetaPoleRemoved (s : ℂ) :
  ...
  have hshift : DifferentiableAt ℂ (fun z : ℂ => s + z) z := by fun_prop
  ...
  have hsubset : u6aShiftedZetaZeroBallFinset s R ⊆ S := by
    ...
    have hD_ne : D z ≠ 0 := by
      have horder_pos : 0 < riemannZeta.order (s + z) :=
        riemannZeta_order_pos_of_zero_ne_one h1 hzmem.2
      rw [hD_eq]
...
/-- Non-trivial zeros whose ordinates lie in the unit window around `T`. -/
def kadiriLocalZeroWindow (T : ℝ) : Set NontrivialZeros :=
  {rho | |(rho : ℂ).im - T| ≤ 1}

theorem kadiriLocalZeroWindow_finite (T : ℝ) :
    (kadiriLocalZeroWindow T).Finite := by
  apply Set.Finite.subset (nontrivialZeros_abs_im_lt_finite (|T| + 2))
  intro rho hrho
  rw [kadiriLocalZeroWindow, Set.mem_setOf_eq] at hrho
  rw [Set.mem_setOf_eq]
  have him_le : |(rho : ℂ).im| ≤ |T| + 1 := by
    have htri := abs_add_le ((rho : ℂ).im - T) T
    have hsplit : (rho : ℂ).im - T + T = (rho : ℂ).im := by ring
    rw [hsplit] at htri
    calc |(rho : ℂ).im| ≤ |(rho : ℂ).im - T| + |T| := htri
      _ ≤ 1 + |T| := by linarith
      _ = |T| + 1 := by ring
  linarith

theorem u6aFTNearbyWindow_ncard_le_nearbyZeroCount {t : ℝ} (ht : 3 ≤ |t|) :
    ((u6aFTNearbyWindow t).ncard : ℝ) ≤ u6aNearbyZeroCount (-1) 2 t := by
  classical
  have hfin := u6aFTNearbyWindow_finite t
  have hcount := riemannZeta.zeroes_sum_eq_finset_of_finite
    (I := Set.uIcc (-1 : ℝ) 2) (J := Set.Icc (t - 1) (t + 1))
    (fun _ => (1 : ℝ)) hfin
  rw [Set.ncard_eq_toFinset_card (u6aFTNearbyWindow t) hfin]
  unfold u6aNearbyZeroCount
  rw [hcount]
  simp only [one_mul]
  calc
    ((hfin.toFinset.card : ℕ) : ℝ)
        = ∑ ρ ∈ hfin.toFinset, (1 : ℝ) := by simp
    _ ≤ ∑ ρ ∈ hfin.toFinset, ((riemannZeta.order ρ : ℤ) : ℝ) := by
        refine Finset.sum_le_sum fun ρ hρ => ?_
        have hρmem : ρ ∈ u6aFTNearbyWindow t := hfin.mem_toFinset.mp hρ
        obtain ⟨_hre, him, hζ⟩ := hρmem
        have himne : ρ.im ≠ 0 := by
          intro h0
          rw [h0] at him
          have habs : |t| ≤ 1 := abs_le.mpr ⟨by linarith [him.2], by linarith [him.1]⟩
        ...
        have hρne1 : ρ ≠ 1 := by
          intro h1
          exact himne (by rw [h1]; simp)
        have hpos := riemannZeta_order_pos_of_zero_ne_one hρne1 hζ
        have hge1 : (1 : ℤ) ≤ riemannZeta.order ρ := by
          simpa using (Int.lt_iff_add_one_le.mp hpos)
        exact_mod_cast hge1

theorem kadiriLocalZeroWindow_natCard_le_nearbyZeroCount {t : ℝ} (ht : 3 ≤ |t|) :
    (Nat.card {rho : NontrivialZeros // |(rho : ℂ).im - t| ≤ 1} : ℝ) ≤
      u6aNearbyZeroCount (-1) 2 t := by
  change ((kadiriLocalZeroWindow t).ncard : ℝ) ≤ u6aNearbyZeroCount (-1) 2 t
  have hcard :
      (kadiriLocalZeroWindow t).ncard ≤ (u6aFTNearbyWindow t).ncard := by
    refine Set.ncard_le_ncard_of_injOn
      (s := kadiriLocalZeroWindow t) (t := u6aFTNearbyWindow t)
      (fun rho : NontrivialZeros => (rho : ℂ)) ?_ ?_ (u6aFTNearbyWindow_finite t)
    · intro rho hrho
      rw [kadiriLocalZeroWindow, Set.mem_setOf_eq] at hrho
      ...
      · rw [Set.uIcc_of_le (by norm_num : (-1 : ℝ) ≤ 2)]
        exact ⟨by linarith [rho.property.1.1], by linarith [rho.property.1.2]⟩
      · exact ⟨by linarith [him_abs.1], by linarith [him_abs.2]⟩
    · intro rho _ eta _ h
      exact Subtype.ext h
  have hcardR :
      ((kadiriLocalZeroWindow t).ncard : ℝ) ≤ ((u6aFTNearbyWindow t).ncard : ℝ) := by
    exact_mod_cast hcard
  exact hcardR.trans (u6aFTNearbyWindow_ncard_le_nearbyZeroCount (t := t) ht)

/-- Local zero-window count: `N(T+1)-N(T-1) = O(log T)` for non-trivial zeros. -/
theorem exists_kadiriLocalZeroWindow_natCard_le_log :
  ...
  have hT_absTmin : |Tₘᵢₙ| ≤ T := by
    calc |Tₘᵢₙ| ≤ max |Tₘᵢₙ| 3 := le_max_left _ _
      _ ≤ max (max |Tₘᵢₙ| 3) 1 := le_max_left _ _
      _ = T0 := rfl
      _ ≤ T := hT
  have hT3 : 3 ≤ T := by
    calc (3 : ℝ) ≤ max |Tₘᵢₙ| 3 := le_max_right _ _
      _ ≤ max (max |Tₘᵢₙ| 3) 1 := le_max_left _ _
      _ = T0 := rfl
      _ ≤ T := hT
  ...
  have hTmin : Tₘᵢₙ ≤ |T| := by
    rw [hTabs]
    exact (le_abs_self Tₘᵢₙ).trans hT_absTmin
  have h3abs : 3 ≤ |T| := by rwa [hTabs]
  calc
    (Nat.card {rho : NontrivialZeros // |(rho : ℂ).im - T| ≤ 1} : ℝ)
        ≤ u6aNearbyZeroCount (-1) 2 T :=
          kadiriLocalZeroWindow_natCard_le_nearbyZeroCount (t := T) h3abs
    _ ≤ C * Real.log |T| := hcnt T hTmin h3abs
    _ = C * Real.log T := by rw [hTabs]
